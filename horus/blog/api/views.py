...
from horus.blog.models import ABCVoting, Comment, Downvote, Love, Post, Upvote
...
class VotingManager(APIView):
    ...
    def get_up_down_vote(cls, post, voter):
        return cls.normalizer_for_related(
            post, voter, Upvote
        ), cls.normalizer_for_related(post, voter, Downvote)

    @classmethod
    def perform_post_voting(
        cls,
        obj: ABCVoting,
        post: Post,
        voter: User,
        successed_created: str,
        successed_removed: str,
        rev: ABCVoting,
    ):
        if obj is None:
            if rev:
            ...
            return Response(
                {"detail": successed_created}, status=status.HTTP_201_CREATED
            )

        obj.delete()
        return Response(
            {"detail": successed_removed}, status=status.HTTP_204_NO_CONTENT
        )
...
class UpAndUnVote(VotingManager):
    ...
    def post(self, request):
        ...
        return self.perform_post_voting(
            upvote,
            post,
            voter,
            successed_created="upvoted",
            successed_removed="un upvoted",
            rev=downvote,
        )
...
class DownAndUnVote(VotingManager):
    ...
    def post(self, request):
        ...
        return self.perform_post_voting(
            ...
            post,
            voter,
            successed_created="downvoted",
            successed_removed="un downvoted",
            rev=upvote,
        )
...
class VotingState(APIView):
    def get(self, request):
        ...
        if post_id is None:
            return Response(
                {"detail": "you should send post_id"},
